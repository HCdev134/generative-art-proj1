...
class rootDrawing{
    ...
    draw(){
        ...
        // decrease pull on x-axis ( make x-axis distance wider)
        this.speedX += (Math.random() - 0.48) / 2;
...
function branchOut(){
    ...
    // increase number of loops, more particles: deathcore band logo
    for(let i =0; i < 4; i++){
        // passing class to root variable 
        // 2nd Accent Color passed into constructor, this.color; 
        const root = new rootDrawing(mouse.x, mouse.y, "#0000002c", centerX, centerY);
        root.draw()
    }
}
